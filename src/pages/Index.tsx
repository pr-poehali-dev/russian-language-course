      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        ...
        <header className="text-center mb-16 animate-fade-in">
          <div className="inline-block mb-4">
            <Badge className="text-lg px-4 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white">
              5 недель • 5 занятий • Онлайн
            </Badge>
          </div>
          <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 bg-clip-text text-transparent">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" className="text-lg px-8 py-6 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 transform hover:scale-105 transition-transform">
              <Icon name="Rocket" className="mr-2" size={24} />
              Начать обучение
            </Button>
            <Button size="lg" variant="outline" className="text-lg px-8 py-6 border-2 border-orange-500 text-orange-400 hover:bg-emerald-800/30 transform hover:scale-105 transition-transform">
        ...
        <section className="mb-20 animate-fade-in">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 bg-gradient-to-r from-orange-500 to-amber-500 bg-clip-text text-transparent">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            {results.map((result, index) => (
              <Card key={index} className="hover:shadow-xl transition-shadow duration-300 border-2 border-emerald-700 hover:border-orange-500 animate-scale-in bg-emerald-900/50" style={{ animationDelay: `${index * 100}ms` }}>
                <CardHeader>
                  <CardTitle className="text-2xl bg-gradient-to-r from-orange-500 to-amber-500 bg-clip-text text-transparent flex items-center gap-2">
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {lessons.map((lesson, index) => (
              <AccordionItem 
                key={index} 
                value={`lesson-${index}`}
                className="border-2 border-emerald-700 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow bg-emerald-900/30"
              >
                <AccordionTrigger className="px-6 py-5 hover:no-underline bg-emerald-900/50 hover:bg-emerald-800/50">
                  <div className="flex items-center gap-4 w-full text-left">
                    <div className={`${lesson.color} rounded-full p-4 flex-shrink-0`}>
                      <Icon name={lesson.icon as any} size={28} className="text-white" />
                    </div>
                ...
                <AccordionContent className="px-6 py-5 bg-gradient-to-br from-emerald-900/70 to-emerald-800/50">
                    ...
                    <div>
                      ...
                      <ul className="space-y-2 ml-7">
                        {lesson.topics.map((topic, idx) => (
                          <li key={idx} className="flex items-start gap-2">
                            <Icon name="ChevronRight" className="text-orange-500 flex-shrink-0 mt-1" size={18} />
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 ml-7">
                        {lesson.tools.map((tool, idx) => (
                          <Badge key={idx} variant="outline" className="px-3 py-1 text-sm border-orange-500 text-orange-400">
                            {tool}
                          </Badge>
                        ))}
                      </div>
        ...
        <section className="mb-20 animate-fade-in">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {tools.map((tool, index) => (
              <Card key={index} className="hover:shadow-xl transition-all duration-300 border-2 border-emerald-700 hover:border-orange-500 hover:-translate-y-1 cursor-pointer animate-scale-in bg-emerald-900/50" style={{ animationDelay: `${index * 100}ms` }}>
                <CardHeader>
                  <div className="flex items-center gap-4">
                    <div className="bg-gradient-to-br from-orange-500/20 to-amber-500/20 rounded-lg p-3">
                      <Icon name={tool.icon as any} size={28} className="text-orange-500" />
                    </div>
                    <CardTitle className="text-xl text-white">{tool.name}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <CardDescription className="text-base text-gray-200">{tool.description}</CardDescription>
                </CardContent>
              </Card>
            ))}
          </div>
        ...
        <section className="text-center py-16 bg-gradient-to-r from-orange-600 via-amber-600 to-yellow-600 rounded-3xl text-white shadow-2xl animate-fade-in">
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button size="lg" className="text-lg px-8 py-6 bg-white text-emerald-900 hover:bg-gray-100 transform hover:scale-105 transition-transform">
